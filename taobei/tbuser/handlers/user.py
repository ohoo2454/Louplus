...
from ..models import (User, UserSchema, WalletTransaction, 
...
def user_list():
	...
	limit = request.args.get(
		'limit', 
		current_app.config['PAGINATION_PER_PAGE'], 
		type=int
	)
...
def user_infos():
	...
	users = {user.id: UserSchema().dump(user) for user in query}
...
def check_password():
	...
	if username is None or password is None:
		return json_response(isCorrent=False)
	...
	isCorrent = user.check_password(password)

	return json_response(isCorrent=isCorrent, 
		                 user=UserSchema().dump(user) if isCorrent else None)
